...
namespace aoc {
  ...
  auto operator!(dir d) -> dir; // inverse (ex: N -> S)
  ...
  struct grid {
    ...
    auto operator()(size_t idx) const -> T& { return data_[idx]; }
    ...
    auto add_row() { if (cols_ == 0) cols_ = data_.size(); }
    auto peek(size_t idx, dir d) const -> size_t {
      if (idx == npos) return npos;
      switch (d) {
        case dir::N: return row(idx) == 0 ? npos : idx - cols_;
        case dir::S: return (idx + cols_) < data_.size() ? idx + cols_ : npos;
        case dir::E: return (idx + 1)%cols_ > idx%cols_ ? idx + 1 : npos;
        case dir::W: return (idx - 1)%cols_ < idx%cols_ ? idx - 1 : npos;
      }
      assert(false);
    }
    ...
    struct iterator {
      ...
      auto idx() const -> size_t { return idx_; }
      ...
      // random access
      auto operator+(difference_type n) const -> iterator { return {g_, idx_ + n}; }
      auto operator-(difference_type n) const -> iterator { return {g_, idx_ - n}; }
    ...
    auto end()   -> iterator { return {*this, data_.size()}; }
    iterator at(size_t row, size_t col) { auto it=begin(); it.set(row, col); return it; }
    auto begin() const -> iterator { return {const_cast<grid&>(*this), 0}; }
    auto end()   const -> iterator { return {const_cast<grid&>(*this), end_(data_)}; }
  };
}
...
#ifdef _GRID_H_IMPL
auto aoc::operator!(aoc::dir d) -> aoc::dir {
  switch (d) {
    case aoc::dir::N: return aoc::dir::S;
    case aoc::dir::S: return aoc::dir::N;
    case aoc::dir::E: return aoc::dir::W;
    case aoc::dir::W: return aoc::dir::E;
  }
  std::abort();
}
auto operator<<(std::ostream &out, typename aoc::grid<char>::iterator const& i) -> std::ostream& {
